...
@section('content_header')
    <h1 class="m-0 text-dark">Edit Data Pengajuan</h1>
@stop
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control @error('tanggal_izin') is-invalid @enderror" id="tanggal_izin" placeholder="Tanggal Izin" name="tanggal_izin" value="{{$pengajuan->tanggal_izin ?? old('tanggal_izin')}}">
                        @error('tanggal_izin') <span class="text-danger">{{$message}}</span> @enderror
                    </div>
                    <div class="form-group">
                        <label for="akhir_izin">Akhir Izin</label>
                        <input type="text" class="form-control @error('akhir_izin') is-invalid @enderror" id="akhir_izin" placeholder="Akhir Izin" name="akhir_izin" value="{{$pengajuan->akhir_izin ?? old('akhir_izin')}}">
                        ...
                        <label for="kode_izin">Kode Izin</label>
                            <select name="kode_izin" id="kode_izin" required class="form-control">
                                @foreach ($jenisijin as $ijin)
                                    ...
                                    @if ($ijin->kode_izin == old('kode_izin', $pengajuan->kode_izin))
                                        selected="selected"
                                    @endif
                                    >{{$ijin->kode_izin}}</option>
                                @endforeach
                           </select>
                        </div>
                    </div>
